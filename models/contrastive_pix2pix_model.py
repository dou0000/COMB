...
class ContrastivePix2PixModel(torch.nn.Module):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument(
            '--num_patches', type=int, default=1024, help='number of patches per layer'
        )
        parser.add_argument(
            '--no_contrastive_loss',
            default=False,
    ...
    def keep_encoder_weights(self, raw_sd, model, prefixes=("enc.", "encoder.")):
        tgt_sd   = model.state_dict()          # shapes in the new model
        filtered = {}

        for k, v in raw_sd.items():
            for p in prefixes:
                if k.startswith(p):
                    # normalise key name so it matches “enc.” in the new model
                    new_k = "enc." + k[len(p):] if p != "enc." else k
                    if new_k in tgt_sd and v.shape == tgt_sd[new_k].shape:
                        filtered[new_k] = v
    ...
    def preprocess_input(self, data):
        ...
        return data['input_patches'], data['target_patches'].squeeze(1)
    ...
    def compute_generator_loss(self, input_semantics, real_image):
        ...
        if isinstance(fake_image, tuple):
            global_fake_image, local_fake_image, fake_image = fake_image
